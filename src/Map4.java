...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        //Getting has of key and determining bucket
        int keyHash = key.hashCode();
        int bucket = mod(keyHash, this.hashTable.length);

        //Increasing size and adding key and value to the bucket
        this.size++;
    ...
    public final Pair<K, V> remove(K key) {
        ...
        // Gets bucket number for key
        int keyHash = key.hashCode();
        int bucket = mod(keyHash, this.hashTable.length);

        // Removes pair and returns it
        this.size--;
    ...
    public final Pair<K, V> removeAny() {
        ...
        //Declaring a starting bucket variable
        int bucketIndex = 0;

        //Entering while loop until finding a bucket that contains something
        while (this.hashTable[bucketIndex].size() == 0) {
            bucketIndex++;
        }

        //Decreasing size and removing, then returning it
        this.size--;
        return this.hashTable[bucketIndex].removeAny();
    }
    ...
    public final boolean hasKey(K key) {
        ...
        //Determining bucket and then getting key value, then returning it
